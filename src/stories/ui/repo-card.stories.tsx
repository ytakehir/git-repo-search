...
const item: RepoSearchResultItem = {
  id: 3081286,
...
export const Default: StoryObj<typeof RepoCard> = {
  render: () => {
    return <RepoCard {...item} />
  },
  play: async ({ canvas }) => {
    await expect(canvas.getByTestId('title')).toBeVisible()
    await expect(canvas.getByTestId('description')).toBeVisible()
    await expect(canvas.getByTestId('star')).toBeVisible()
    await expect(canvas.getByTestId('fork')).toBeVisible()
    await expect(canvas.getByTestId('issue')).toBeVisible()

    await expect(canvas.getByText('dtrupenn/Tetris')).toBeVisible()
    await expect(canvas.getByText('A C implementation of Tetris using Pennsim through LC4')).toBeVisible()
